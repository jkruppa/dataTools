                           ...
                           illuminaclip, log_file) 
    {
      if(length(inFile) == 2) {
        ...
        fastq_trimmer_CMD <- paste("java -jar", trimmomatic, 
            ...
            paired_outFile["R2"], unpaired_outFile["R2"], str_c("ILLUMINACLIP:", 
                illuminaclip, ":2:30:10"), str_c("LEADING:", 
                leading), str_c("TRAILING:", trailing), str_c("MINLEN:", 
                minlength), "SLIDINGWINDOW:4:15",
            "2>", log_file)
    }
    else {
        fastq_trimmer_CMD <- paste("java -jar", trimmomatic, 
            "SE", "-threads", par$nCores, "-phred33", inFile, 
            outFile, str_c("ILLUMINACLIP:", illuminaclip, ":2:30:10"), 
            str_c("LEADING:", leading), str_c("TRAILING:", trailing), 
            str_c("MINLEN:", minlength), "SLIDINGWINDOW:4:15",
            "2>", log_file)
    }
    runCMD(fastq_trimmer_CMD)
}

##' Main fastq quality function [deprecated]
##'
...
##' @author Jochen Kruppa
##' @export
fastq_quality_control_old <- function (inFile, tmpDir, p = 20, q = 50, leading = 10, 
